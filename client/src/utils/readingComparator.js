/**
 * Reading Comparator & Phonics Diagnostics Engine
 * Evaluates spoken text against original sentence to detect:
 * - Substitutions (wrong words)
 * - Omissions (skipped words)
 * - Insertions (extra words)
 * - Cognitive Dyslexia Phonics Confusion (vowels, mirror letters, sight words)
 */
...
/**
 * Align Original Words and Spoken Words using Dynamic Programming
 */
export function alignReading(originalText, spokenText) {
  ...
  // Cost matrix for DP alignment
  const dp = Array.from({ length: m + 1 }, () => new Array(n + 1).fill(0));

  for (let i = 0; i <= m; i++) dp[i][0] = i * 2; // Omission cost
  for (let j = 0; j <= n; j++) dp[0][j] = j * 2; // Insertion cost
  ...
  for (let i = 1; i <= m; i++) {
    for (let j = 1; j <= n; j++) {
      ...
      dp[i][j] = Math.min(
        ...
        dp[i - 1][j] + 2,             // Omission (orig skipped)
        dp[i][j - 1] + 2              // Insertion (extra spoken)
      );
    }
  }
...
/**
 * Detects Dyslexia-specific cognitive phonics confusion patterns
 */
export function analyzeDyslexicPatterns(alignment) {
  ...
  alignment.forEach((item) => {
    if (item.type === 'SUBSTITUTION' && item.original && item.spoken) {
      ...
      // 1. Mirror Letter Confusion (b/d, p/q, m/w, n/u)
      const mirrorPairs = [
        ['b', 'd'],
        ['p', 'q'],
        ['m', 'w'],
      ...
      // 2. Short Vowel Confusion (a, e, i, o, u)
